...
def _check_same_length(x: Iterable[float], y: Iterable[float]) -> Tuple[List[float], List[float]]:
    x = list(x)
    y = list(y)
    if len(x) != len(y):
...
def evaluate_all(pred, gt):
    ...
    pairs = [(float(a), float(b)) for a, b in zip(pred, gt) if (a == a and b == b)]  # drop NaN
    if len(pairs) < 2:
